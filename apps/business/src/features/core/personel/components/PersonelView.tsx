...
import { Drawer, DrawerContent, DrawerHeader, DrawerTitle, DrawerClose } from '@adatrack/ui';
...
export function PersonelView({ personel, open, onClose, onEdit, onDelete, labels }: PersonelViewProps) {
  ...
  return (
    <Drawer open={open} onOpenChange={(val) => !val && onClose()}>
      <DrawerContent className="w-full sm:max-w-md h-[100dvh] flex flex-col p-0">
        <DrawerHeader className="p-4 border-b border-border flex items-center justify-between sticky top-0 bg-background z-10">
        ...
        <div className="p-4 border-t border-border bg-background flex gap-3 sticky bottom-0 z-10 mt-auto">
          <Button variant="outline" className="flex-1 text-destructive hover:bg-destructive/10 hover:text-destructive border-destructive/20" onClick={onDelete}>
