...
export function MileageTable({
  ...
  const handleExportExcel = () => {
    const headers = [
      ...
      locale === 'en' ? 'Total Engine ON' : 'Total Mesin Hidup',
      locale === 'en' ? 'Total KM' : 'Total KM',
    ];
    ...
    const dataRows = filteredVehicles.map((v, idx) => {
      return [
        ...
        toCsvCell('0 j 0 m'), // Placeholder for exported data
        toCsvCell('0 km'),
      ].join(',');
    });
    ...
    link.href = url;
    link.download = `Kilometer_Summary_${new Date().toISOString().slice(0,10)}.csv`;
          ...
          <table className="w-full text-left border-collapse min-w-[900px]">
            <thead className="sticky top-0 bg-neutral-100 dark:bg-neutral-800/80 backdrop-blur-sm border-b border-border z-10">
              <tr>
                ...
                <th className="px-2.5 py-2 text-[12px] font-semibold text-foreground-muted whitespace-nowrap text-right">{locale === 'en' ? 'Total Engine ON' : 'Total Mesin Hidup'}</th>
                <th className="px-2.5 py-2 text-[12px] font-semibold text-foreground-muted whitespace-nowrap text-right">{locale === 'en' ? 'Total KM' : 'Total KM'}</th>
              </tr>
            </thead>
